...
def calculate_demographic_data(print_data=True):
  ...
  rich_percentage = round(num_min_workers / df['hours-per-week'].value_counts()[min_work_hours] * 100, 1)

  # What country has the highest percentage of people that earn >50K?
  highSalaryByCountry = df.groupby('native-country')['salary'].value_counts(normalize=True).loc[:, '>50K']
  highest_earning_country = highSalaryByCountry.idxmax()
  highest_earning_country_percentage = round(highSalaryByCountry.max() * 100, 1)
